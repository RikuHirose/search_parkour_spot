  ...
  if (window.File && window.FileReader && this.files) {
    ...
    for (var i = 0, I = this.files.length; i < I; i++) {
      ...
      reader.onload = function (file, i) { return function () {
        var tag = '<img src="'+ this.result +'" alt="'+ file.name +'" title="'+ file.name +' ('+ Math.round( file.size / 1024 ) +'kb)' +'" class="thumb thumbpos" />';
      ...
      } else if (file && (file.type && file.type.match(/^video/)    // video ?
       || !file.type && file.name.match(/\.(mp4+|qt|x-ms-wmv|mpeg|x-msvideo|MOV)$/i) && $.browser.msie)) {
        var reader = new FileReader();
        reader.onload = function (file, i) { return function () {
          // console.log(reader.result)
          createThumb(this.result)
            .then(function (imageUrl) {
            var tag = '<img src="'+ imageUrl +'" alt="'+ file.name +'" title="'+ file.name +' ('+ Math.round( file.size / 1024 ) +'kb)' +'" class="thumb  thumbpos"  />';
              // set thumbnail images
              $.fn.uploadThumbs.set.call($self, option, tag);
            })
        }}(file, i);
        // reader.readAsDataURL(file);  // read video data
...
$.fn.uploadThumbs.clear = function (option) {
  ...
  // find videos
  var $images = (typeof option.position == 'number')
  ?  this.siblings('video.thumb')
  :  $( option.position ).find('video.thumb');

  // remove videos
  if ($images.length) {
    $images.not('video.uploaded').next('br').remove()
    .end().remove();
  }
};
